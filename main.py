...
async def handle_pesan(update: Update, context: CallbackContext):
    ...
    # --- PROSES MENFESS ---
        ...
        # Pola Regex anti-bypass (menangkap huruf yang diselipi spasi/segala jenis tanda baca/emoji)
...
async def live_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Memaksa bot membaca file ffmpeg di folder saat ini (current working directory)
    ffmpeg_exe = "./ffmpeg" 
    
    # Tambahkan pengecekan manual apakah file tersebut benar-benar ada
    import os
    if not os.path.isfile(ffmpeg_exe):
        return await msg.reply_text(
            "❌ File FFmpeg static tidak ditemukan di folder bot.",
            reply_markup=get_main_keyboard(),
        )
