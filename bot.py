...
CACHE_HASHTAGS = []
required_channels = []
CACHE_BANNED_USERS = set()
CACHE_COMSECT_OFF = set()
CACHE_BAD_WORDS = set()
BAD_WORDS_RE = None

DB_TIMEOUT = int(os.environ.get('DB_TIMEOUT', '12'))
SUBSCRIPTION_CACHE = {}
SUBSCRIPTION_OK_TTL = int(os.environ.get('SUBSCRIPTION_OK_TTL', '300'))
SUBSCRIPTION_FAIL_TTL = int(os.environ.get('SUBSCRIPTION_FAIL_TTL', '30'))
LIVE_CONCURRENCY = max(1, int(os.environ.get('LIVE_CONCURRENCY', '1')))
LIVE_SEMAPHORE = asyncio.Semaphore(LIVE_CONCURRENCY)
...
def _make_supabase_client():
    ...
    option_attempts = (
        {'postgrest_client_timeout': DB_TIMEOUT, 'storage_client_timeout': 30, 'schema': 'public'},
        {'postgrest_client_timeout': DB_TIMEOUT, 'schema': 'public'},
        {'schema': 'public'},
    )
    for kwargs in option_attempts:
        try:
            return create_client(SUPABASE_URL, SUPABASE_KEY, options=ClientOptions(**kwargs))
        except TypeError:
            continue
...
async def db_exec(fn, timeout=None):
    """Run blocking Supabase/PostgREST work outside the event loop."""
    return await asyncio.wait_for(asyncio.to_thread(fn), timeout=timeout or DB_TIMEOUT)
...
def _channel_public_name(channel_id):
    return str(channel_id or '').lstrip('@')


def _main_menu_text():
    return '👋 <b>Navigasi Bot</b>\n\nSilakan pilih menu di bawah:'


async def _notify_admin_error(context, text):
    if not LOG_GROUP_ID:
        return
    try:
...
async def update_settings_cache():
    global MENFESS_MODE
    try:
        response = await db_exec(lambda: supabase.table('bot_settings').select('value').eq('key', 'menfess_mode').execute())
        if hasattr(response, 'data') and response.data:
            MENFESS_MODE = response.data[0]['value']
        else:
            await db_exec(lambda: supabase.table('bot_settings').insert({'key': 'menfess_mode', 'value': 'auto'}).execute())
            MENFESS_MODE = 'auto'
    except Exception as e:
...
async def update_hashtags_cache():
    ...
    try:
        response = await db_exec(lambda: supabase.table('triggered_hashtags').select('hashtag').eq('active', True).execute())
...
async def update_badwords_cache():
    global CACHE_BAD_WORDS, BAD_WORDS_RE
    try:
        response = await db_exec(lambda: supabase.table('bad_words').select('word').execute())
        CACHE_BAD_WORDS = {str(row['word']).lower() for row in response.data if row.get('word')} if hasattr(response, 'data') and response.data else set()
        if CACHE_BAD_WORDS:
            pattern = '|'.join(re.escape(word) for word in sorted(CACHE_BAD_WORDS, key=len, reverse=True) if word)
            BAD_WORDS_RE = re.compile(rf'(?<!\w)({pattern})(?!\w)', re.IGNORECASE)
        else:
            BAD_WORDS_RE = None
    except Exception as e:
...
async def update_required_channels_cache():
    ...
    try:
        response = await db_exec(lambda: supabase.table('required_channels').select('channel_username').execute())
...
async def update_banned_users_cache():
    ...
    try:
        response = await db_exec(lambda: supabase.table('banned_users').select('user_id').execute())
        CACHE_BANNED_USERS = {int(row['user_id']) for row in response.data if row.get('user_id') is not None} if hasattr(response, 'data') and response.data else set()
    except Exception as e:
...
async def save_required_channels(channels):
    try:
        await db_exec(lambda: supabase.table('required_channels').delete().neq('channel_username', '').execute())
        for channel in channels:
            await db_exec(lambda: supabase.table('required_channels').insert({'channel_username': channel}).execute())
    except Exception as e:
...
async def check_subscription(user_id, context: CallbackContext):
    ...
    results = await asyncio.gather(*(check_one(channel) for channel in required_channels), return_exceptions=False)
    ok = all(results)
    SUBSCRIPTION_CACHE[user_id] = {
        'ok': ok,
        'expires_at': now + (SUBSCRIPTION_OK_TTL if ok else SUBSCRIPTION_FAIL_TTL),
    }
    return ok
async def add_kith_coins(user_id: int, amount: int):
    try:
        response = await db_exec(lambda: supabase.table('users').select('kith_coins, total_kith_coins').eq('user_id', user_id).execute())
        ...
        await db_exec(lambda: supabase.table('users').update({'kith_coins': new_balance, 'total_kith_coins': new_total}).eq('user_id', user_id).execute())
        return new_balance
    except Exception as e:
...
async def cek_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        res = await db_exec(lambda: supabase.table('users').select('kith_coins, total_kith_coins').eq('user_id', user_id).execute())
        ...
        text = (
            ...
            f'🏷️ Username: @{h(username)}\n'
            f'🪙 Saldo Kith-Coins: <b>{coins}</b>\n'
            f'🏆 Total Koin Diperoleh: <b>{total_coins}</b>\n'
        )
        await update.message.reply_text(text, parse_mode='HTML')
    except Exception as e:
        logger.error(f'Gagal mengambil data profil untuk {user_id}: {e}')
        await update.message.reply_text('❌ Gagal mengambil data profil.')
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        res = await db_exec(lambda: supabase.table('users').select('user_id, kith_coins, total_kith_coins').order('total_kith_coins', desc=True).limit(10).execute())
        if not res.data:
            return await update.message.reply_text('Belum ada data pemain.')

        lines = ['🏆 <b>LEADERBOARD TOTAL KITH-COINS</b> 🏆', '']
        for i, row in enumerate(res.data, start=1):
            ...
            try:
                ...
                display_name = f'@{chat.username}' if chat.username else (chat.first_name or f'User ID: {user_id}')
            except Exception:
                display_name = f'👤 User ID: {user_id}'
            lines.append(f'{i}. {h(display_name)} - <b>{coins}</b> Coins')

        lines.append('')
        lines.append('Leaderboard dihitung dari total koin yang pernah diperoleh, bukan saldo saat ini.')
...
async def buy_title(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        response = await db_exec(lambda: supabase.table('users').select('kith_coins').eq('user_id', user_id).execute())
        ...
        await db_exec(lambda: supabase.table('users').update({'kith_coins': new_balance}).eq('user_id', user_id).execute())
        try:
            await context.bot.set_chat_member_tag(chat_id=GROUP_ID_DISKUSI, user_id=user_id, tag=new_title)
            await update.message.reply_text(f'✅ Transaksi Berhasil!\n\n🏷️ Title barumu: `{new_title}`\n🪙 Sisa saldo Kith-Coins: {new_balance}\n\nSilakan kirim pesan di grup diskusi untuk melihat title barumu!', parse_mode='Markdown')
        except Exception as telegram_err:
            await db_exec(lambda: supabase.table('users').update({'kith_coins': current_balance}).eq('user_id', user_id).execute())
...
async def _apply_title_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE, new_title: str):
    ...
    try:
        ...
        try:
            await context.bot.set_chat_member_tag(chat_id=GROUP_ID_DISKUSI, user_id=user_id, tag=new_title)
            await update.message.reply_text(f'✅ Transaksi Berhasil!\n\n🏷️ Title barumu: `{new_title}`\n🪙 Sisa saldo Kith-Coins: {new_balance}\n\nSilakan kirim pesan di grup diskusi untuk melihat title barumu!', parse_mode='Markdown', reply_markup=get_main_keyboard())
        except Exception as telegram_err:
            await db_exec(lambda: supabase.table('users').update({'kith_coins': current_balance}).eq('user_id', user_id).execute())
            logger.error(f'Gagal set title Telegram: {telegram_err}')
            if 'User_Not_Participant' in str(telegram_err) or 'user is not a member' in str(telegram_err):
...
async def add_badwords(update: Update, context: CallbackContext):
    ...
    for w in words:
        if w:
            try:
                await db_exec(lambda: supabase.table('bad_words').upsert({'word': w}).execute())
                inserted += 1
            except Exception:
                pass
    await update_badwords_cache()
...
async def remove_badwords(update: Update, context: CallbackContext):
    ...
    for w in words:
        if w:
            try:
                await db_exec(lambda: supabase.table('bad_words').delete().eq('word', w).execute())
                deleted += 1
            except Exception:
                pass
    await update_badwords_cache()
...
async def block_user(update: Update, context: CallbackContext):
    ...
    try:
        ...
        await db_exec(lambda: supabase.table('banned_users').upsert({'user_id': target_id}).execute())
        await update_banned_users_cache()
...
async def unblock_user(update: Update, context: CallbackContext):
    ...
    try:
        target_id = int(context.args[0])
        await db_exec(lambda: supabase.table('banned_users').delete().eq('user_id', target_id).execute())
        await update_banned_users_cache()
...
async def set_mode_auto(update: Update, context: CallbackContext):
    ...
    MENFESS_MODE = 'auto'
    try:
        await db_exec(lambda: supabase.table('bot_settings').upsert({'key': 'menfess_mode', 'value': 'auto'}).execute())
    except Exception as e:
...
async def set_mode_manual(update: Update, context: CallbackContext):
    ...
    MENFESS_MODE = 'manual'
    try:
        await db_exec(lambda: supabase.table('bot_settings').upsert({'key': 'menfess_mode', 'value': 'manual'}).execute())
    except Exception as e:
...
async def add_hashtag(update: Update, context: CallbackContext):
    ...
    await db_exec(lambda: supabase.table('triggered_hashtags').upsert({'hashtag': hashtag}).execute())
    await update_hashtags_cache()
...
async def remove_hashtag(update: Update, context: CallbackContext):
    ...
    hashtag = context.args[0].strip()
    await db_exec(lambda: supabase.table('triggered_hashtags').delete().eq('hashtag', hashtag).execute())
    await update_hashtags_cache()
...
async def enable_hashtag(update: Update, context: CallbackContext):
    ...
    hashtag = context.args[0].strip()
    await db_exec(lambda: supabase.table('triggered_hashtags').update({'active': True}).eq('hashtag', hashtag).execute())
    await update_hashtags_cache()
...
async def disable_hashtag(update: Update, context: CallbackContext):
    ...
    hashtag = context.args[0].strip()
    await db_exec(lambda: supabase.table('triggered_hashtags').update({'active': False}).eq('hashtag', hashtag).execute())
    await update_hashtags_cache()
...
async def save_user(user_id, username):
    try:
        await db_exec(lambda: supabase.table('users').upsert({'user_id': user_id, 'username': username}, on_conflict=['user_id']).execute())
    except Exception:
        pass

def get_main_keyboard():
    keyboard = [[KeyboardButton('💬 Beli title'), KeyboardButton('💌 Menfess')], [KeyboardButton('👤 Profile'), KeyboardButton('📸 Photo live')]]
...
async def start(update: Update, context: CallbackContext):
    ...
    if await check_subscription(user_id, context):
        await update.message.reply_text('Halo Kens, selamat datang di *Kitheons*! ☕️\n\n𔐼 *Kitheons:* [@kitheons](https://t.me/kitheons)\n𔐼 *Ch Arsip:* [@kithives](https://t.me/kithives)\n\nKetuk /menu untuk menampilkan navigasi.\n*(Semua pesan yang kamu kirim otomatis diajukan sebagai menfess)*', parse_mode='Markdown', reply_markup=get_main_keyboard())
    else:
...
async def handle_pesan(update: Update, context: CallbackContext):
    ...
    if update.message.reply_to_message:
        ...
        match = re.search('#ID:(\\d+)', replied_text)
        if match:
            try:
    ...
    if update.message.text == '💬 Beli title':
        ...
        await update.message.reply_text(f'🛒 *Beli Custom Title*\nHarga: *{TITLE_PRICE} Kith-Coins*.\n\nKetik *Nama Title Barumu* (Maks 16 karakter):', parse_mode='Markdown', reply_markup=get_cancel_keyboard())
        return ConversationHandler.END
    if update.message.text == '📸 Photo live':
        context.user_data['keyboard_state'] = KEYBOARD_STATE_LIVE
        await update.message.reply_text(f'📸 *Buat Photo Live*\nBiaya: *{LIVE_PHOTO_PRICE} Kith-Coins*\n\nKirim/forward videonya ke sini (maks {LIVE_MAX_DURATION:.0f} detik, input maksimal {LIVE_MAX_INPUT_FILE_SIZE_MB} MB).', parse_mode='Markdown', reply_markup=get_cancel_keyboard())
        return ConversationHandler.END
    if update.message.text == '💌 Menfess':
        ...
        return ConversationHandler.END
    if BAD_WORDS_RE and BAD_WORDS_RE.search(pesan_teks):
    ...
    if MENFESS_MODE == 'auto':
        ...
        if update.message.entities:
            for ent in update.message.entities:
                if ent.type == 'mention':
                    ada_mention = True
                    break
        if ada_mention or re.search('(?:^|\\s)@/?\\w+', pesan_teks):
        ...
        try:
            ...
            keyboard = [[InlineKeyboardButton('✅ Acc (CS ON)', callback_data=f'mf|A_ON|{user_id}|{update.message.message_id}'), InlineKeyboardButton('🔕 Acc (CS OFF)', callback_data=f'mf|A_OFF|{user_id}|{update.message.message_id}')], [InlineKeyboardButton('❌ Tolak', callback_data=f'mf|R|{user_id}|{update.message.message_id}')]]
            review_text = f'🚨 <b>REVIEW MENFESS</b>\n👤 Pengirim: {h(display_name)}\n🆔 ID: <code>{user_id}</code>'
            await context.bot.send_message(chat_id=ADMIN_GROUP_ID, text=review_text, reply_to_message_id=fw_msg.message_id, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='HTML')
...
async def handle_username(update: Update, context: CallbackContext):
    ...
    if raw in {'❌ Cancel', '/cancel'}:
        return await cancel_menfess(update, context)

    if raw in {'👤 Profile', '💬 Beli title', '💌 Menfess', '📸 Photo live'}:
        await update.message.reply_text(
            '⚠️ Kamu sedang di tahap kirim username untuk hyperlink menfess.\n'
            'Kirim username tanpa spasi, contoh: @jake\n\n'
            'Ketik /cancel untuk membatalkan.'
        )
        return WAITING_USERNAME

    target_username = raw.replace('@', '')
    if not re.fullmatch(r'[A-Za-z0-9_]{5,32}', target_username):
        await update.message.reply_text(
            '❌ Username tidak valid.\n'
            'Kirim username Telegram tanpa spasi, contoh: @jake\n\n'
            'Ketik /cancel untuk membatalkan.'
        )
        return WAITING_USERNAME
    ...
    if not teks_asli:
        ...
        await update.message.reply_text('❌ Sesi menfess sudah kedaluwarsa. Silakan kirim ulang menfess kamu.', reply_markup=get_main_keyboard())
        return ConversationHandler.END
    ...
    try:
        ...
        keyboard = [[InlineKeyboardButton('Lihat Pesan Kamu', url=f'https://t.me/{_channel_public_name(CHANNEL_ID)}/{message_sent.message_id}')]]
        await update.message.reply_text(f'Pesan kamu telah dikirim ke channel! 🪶{coin_msg}', reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='HTML')

        try:
            await db_exec(lambda: supabase.table('menfess_map').insert({'post_id': message_sent.message_id, 'sender_user_id': user_id}).execute())
        except Exception as e:
            logger.error(f'DB Error Auto: {e}')

        log_msg = (
            '📌 <b>Log Menfess (AUTO)</b>\n'
            f'🕰️ Waktu: {h(update.message.date)}\n'
            f'👤 Pengirim: {h(display_name)}\n'
            f'🆔 ID: <code>{user_id}</code>\n'
            f'🔗 Username Target: @{h(target_username)}\n'
            f'💬 Pesan: {h(teks_asli)}'
        )
        await context.bot.send_message(
            chat_id=LOG_GROUP_ID,
            text=log_msg,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('🔍 Lihat Pesan', url=f'https://t.me/{_channel_public_name(CHANNEL_ID)}/{message_sent.message_id}')]]),
            parse_mode='HTML',
        )
    except Exception as e:
    ...
    if data.startswith('mf|'):
        ...
        if action in ['A_ON', 'A_OFF']:
            ...
            try:
                ...
                try:
                    await db_exec(lambda: supabase.table('menfess_map').insert({'post_id': sent_msg.message_id, 'sender_user_id': user_id}).execute())
                except Exception as e:
...
async def handle_admin_reply(update: Update, context: CallbackContext):
    if update.effective_chat.id not in [ADMIN_GROUP_ID, LOG_GROUP_ID] or not update.message.reply_to_message:
        return
    match = re.search('ID(?:\\s*Pengguna)?:?\\s*[`]*(\\d+)', update.message.reply_to_message.text or update.message.reply_to_message.caption or '')
    if not match:
        return
    ...
    if reply_text and reply_text.startswith('/'):
        try:
            response = await db_exec(lambda: supabase.table('commands').select('content').eq('name', reply_text.split()[0]).execute())
            if hasattr(response, 'data') and response.data:
...
async def handle_discussion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.is_automatic_forward and msg.forward_origin and (msg.forward_origin.type == 'channel'):
        ...
        if origin_chat.username and '@' + origin_chat.username.lower() == CHANNEL_ID.lower():
            try:
                await db_exec(lambda: supabase.table('menfess_map').update({'discussion_message_id': msg.message_id}).eq('post_id', post_id).execute())
            except Exception:
                pass
        return
    if msg.reply_to_message:
        try:
            ...
            response = await db_exec(lambda: supabase.table('menfess_map').select('sender_user_id, post_id').eq('discussion_message_id', replied_msg_id).execute())
            if hasattr(response, 'data') and response.data:
                ...
                notif_text = f'📬 {commenter} berkomentar di menfess kamu!\n\n*(balas/reply pesan ini jika kamu ingin membalas komentarnya secara anonim)*\n\n`#ID:{msg.message_id}`'
...
async def get_all_user_ids():
    try:
        response = await db_exec(lambda: supabase.table('users').select('user_id').execute())
...
async def menu(update: Update, context: CallbackContext):
    if update.effective_chat.type != 'private':
        return
    await update.message.reply_text('👋 <b>Navigasi Bot</b>\n\nSilakan pilih menu di bawah:', parse_mode='HTML', reply_markup=get_main_keyboard())
...
async def broadcast_forward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text('⏳ Broadcast forward dimulai di background. Bot tetap bisa dipakai.')
    context.application.create_task(
        _broadcast_forward_job(context, update.effective_chat.id, channel_username, int(message_id))
    )


async def _broadcast_forward_job(context: ContextTypes.DEFAULT_TYPE, admin_chat_id: int, channel_username: str, message_id: int):
    ...
    for i, user_id in enumerate(user_list, start=1):
        try:
            await context.bot.forward_message(chat_id=user_id, from_chat_id=f'@{channel_username}', message_id=message_id)
            sc += 1
        except Exception:
            fc += 1
        ...
        if i % 100 == 0:
            try:
                await context.bot.send_message(admin_chat_id, f'📣 Progress broadcastfw: {i}/{total} | Berhasil: {sc}, Gagal: {fc}')
            except Exception:
                pass

    await context.bot.send_message(admin_chat_id, f'✅ Broadcast forward selesai! Berhasil: {sc}, Gagal: {fc}')
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.id != ADMIN_GROUP_ID or not context.args:
        return await update.message.reply_text('Format: /broadcast <teks>')

    message_text = ' '.join(context.args)
    await update.message.reply_text('⏳ Broadcast dimulai di background. Bot tetap bisa dipakai.')
    context.application.create_task(
        _broadcast_job(context, update.effective_chat.id, message_text)
    )


async def _broadcast_job(context: ContextTypes.DEFAULT_TYPE, admin_chat_id: int, message_text: str):
    user_list = await get_all_user_ids()
    sc, fc = 0, 0
    total = len(user_list)
    ...
    for i, user_id in enumerate(user_list, start=1):
        ...
        if i % 100 == 0:
            try:
                await context.bot.send_message(admin_chat_id, f'📣 Progress broadcast: {i}/{total} | Berhasil: {sc}, Gagal: {fc}')
            except Exception:
                pass

    await context.bot.send_message(admin_chat_id, f'✅ Broadcast selesai! Berhasil: {sc}, Gagal: {fc}')
async def add_command(update: Update, context: CallbackContext) -> None:
    ...
    try:
        await db_exec(lambda: supabase.table('commands').upsert({'name': command_name, 'content': command_content}).execute())
...
async def delete_command(update: Update, context: CallbackContext) -> None:
    ...
    try:
        await db_exec(lambda: supabase.table('commands').delete().eq('name', command_name).execute())
...
async def add_uc_word(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await db_exec(lambda: supabase.table('uc_words').insert({'word1': w1, 'word2': w2}).execute())
...
async def submit_word(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    res = await db_exec(lambda: supabase.table('uc_active_games').select('*').eq('status', 'playing').execute())
    if not res.data:
        return
    ...
    await db_exec(lambda: supabase.table('uc_active_games').update({'players': players}).eq('game_id', game['game_id']).execute())
...
async def get_discussion_link(comment_msg_id: int, thread_id: int=None):
    ...
    try:
        ...
        map_res = await db_exec(lambda: supabase.table('menfess_map').select('post_id').eq('discussion_message_id', lookup_id).execute())
...
async def start_undercover(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text(f'🕵️\u200d♂️ *GAME UNDERCOVER*\n\n👑 Room Master: {creator_name}\n\n👥 *Pemain Terdaftar:*\n1. {creator_name} (@{creator_username})\n\n*(Minimal 3 pemain)*', reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    players_data = {str(creator_id): {'name': creator_name, 'username': creator_username, 'current_word': ''}}
    try:
        await db_exec(lambda: supabase.table('uc_active_games').insert({'game_id': msg.message_id, 'chat_id': update.effective_chat.id, 'status': 'lobby', 'creator_id': creator_id, 'players': players_data, 'undercover_id': 0, 'civilian_word': '', 'undercover_word': '', 'votes': {}}).execute())
    except Exception as e:
...
async def handle_uc_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    res = await db_exec(lambda: supabase.table('uc_active_games').select('*').eq('game_id', game_id).execute())
    ...
    if query.data == 'uc_join':
        ...
        await db_exec(lambda: supabase.table('uc_active_games').update({'players': players}).eq('game_id', game_id).execute())
        ...
        words_res = await db_exec(lambda: supabase.table('uc_words').select('*').execute())
        ...
        await db_exec(lambda: supabase.table('uc_active_games').update({'status': 'playing', 'undercover_id': int(undercover_id), 'civilian_word': word_pair['word1'], 'undercover_word': word_pair['word2']}).eq('game_id', game_id).execute())
        ...
        if i < 5:
            ...
            for pid in players.keys():
                try:
                    ...
                    pass
    await db_exec(lambda: supabase.table('uc_active_games').update({'status': 'voting'}).eq('game_id', game_id).execute())
...
async def sus_vote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    res = await db_exec(lambda: supabase.table('uc_active_games').select('*').eq('status', 'voting').execute())
    if not res.data:
        return await update.message.reply_text('❌ Tidak ada sesi voting yang aktif untukmu.')
    game = res.data[0]
    players = game['players']
    if voter_id not in players:
        return await update.message.reply_text('❌ Kamu tidak bermain di game ini.')
    target_id = None
    for pid, pdata in players.items():
    ...
    await db_exec(lambda: supabase.table('uc_active_games').update({'votes': votes}).eq('game_id', game['game_id']).execute())
...
async def tally_votes(chat_id, game_id, thread_id, context):
    ...
    for pid in players.keys():
        ...
        try:
            ...
            pass
    await db_exec(lambda: supabase.table('uc_active_games').delete().eq('game_id', game_id).execute())
...
async def reveal_role(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    koin_res = await db_exec(lambda: supabase.table('users').select('kith_coins').eq('user_id', int(user_id)).execute())
    ...
    await db_exec(lambda: supabase.table('users').update({'kith_coins': saldo - 500}).eq('user_id', int(user_id)).execute())
    undercover_name = game['players'][str(game['undercover_id'])]['name']
    civ_word = game['civilian_word']
    und_word = game['undercover_word']
    await update.message.reply_text(f'🔮 *REVEAL ROLE (Premium)* 🔮\n\n🤫 Undercover: *{undercover_name}*\n📝 Kata Civilian: *{civ_word}*\n📝 Kata Undercover: *{und_word}*\n\n*(Ssstt.. Saldo Koinmu dipotong 500)*', parse_mode='Markdown')
...
async def _send_live_photo_direct(bot_token, chat_id, video_path, photo_path, message_thread_id=None, reply_to_message_id=None):
    ...
    if reply_to_message_id:
        data['reply_parameters'] = json.dumps({'message_id': reply_to_message_id, 'allow_sending_without_reply': True})
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        with open(video_path, 'rb') as vf, open(photo_path, 'rb') as pf:
            files = {'live_photo': ('live_photo.mp4', vf, 'video/mp4'), 'photo': ('photo.jpg', pf, 'image/jpeg')}
...
async def live_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.message
    if not _get_video_file_from_message(msg):
        return await msg.reply_text('Silakan kirim video terlebih dahulu.', reply_markup=get_main_keyboard())

    await msg.reply_text('📥 Video diterima. Masuk antrean proses Live Photo.', reply_markup=get_main_keyboard())
    context.application.create_task(
        _live_photo_worker(update, context)
    )


async def _live_photo_worker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with LIVE_SEMAPHORE:
        await _live_photo_process(update, context)


async def _live_photo_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.message
    user_id = update.effective_user.id
    ...
    if not ffmpeg_exe:
        return await msg.reply_text('❌ FFmpeg belum kebaca di server.', reply_markup=get_main_keyboard())
    ...
    charged = False
    try:
        res = await db_exec(lambda: supabase.table('users').select('kith_coins').eq('user_id', user_id).execute())
        ...
        if LIVE_PHOTO_PRICE > 0 and current_balance < LIVE_PHOTO_PRICE:
            return await msg.reply_text(f'❌ Kith-Coins kurang (Biaya: {LIVE_PHOTO_PRICE} Coins). Saldo: {current_balance}', reply_markup=get_main_keyboard())
        if LIVE_PHOTO_PRICE > 0:
            await db_exec(lambda: supabase.table('users').update({'kith_coins': current_balance - LIVE_PHOTO_PRICE}).eq('user_id', user_id).execute())
            charged = True
    except Exception as e:
    ...
    try:
        ...
        attempts = [{'width': 720, 'vb': '3500k', 'mr': '4200k', 'bs': '8400k', 'ab': '96k'}, {'width': 540, 'vb': '2200k', 'mr': '2600k', 'bs': '5200k', 'ab': '80k'}, {'width': 480, 'vb': '1400k', 'mr': '1700k', 'bs': '3400k', 'ab': '64k'}]
        last_error = ''
        encoded_ok = False
        for attempt in attempts:
            try:
                ...
                await _run_cmd([ffmpeg_exe, '-hide_banner', '-loglevel', 'error', '-i', input_path, '-t', str(LIVE_MAX_DURATION), '-map', '0:v:0', '-map', '0:a?', '-vf', vf, '-c:v', 'libx264', '-preset', 'veryfast', '-b:v', attempt['vb'], '-maxrate', attempt['mr'], '-bufsize', attempt['bs'], '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', attempt['ab'], '-movflags', '+faststart', '-y', output_live_photo], timeout=240)
        ...
        await _run_cmd([ffmpeg_exe, '-hide_banner', '-loglevel', 'error', '-ss', '0.2', '-i', output_live_photo, '-vframes', '1', '-q:v', '2', '-y', output_photo], timeout=120)
        ...
        await _send_live_photo_direct(BOT_TOKEN, msg.chat_id, output_live_photo, output_photo, getattr(msg, 'message_thread_id', None), msg.message_id)
...
async def handle_custom_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        response = await db_exec(lambda: supabase.table('commands').select('content').eq('name', command_name).execute())
        if hasattr(response, 'data') and response.data:
...
async def refresh_total_coin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text('⏳ Refresh total coin dimulai di background. Bot tetap bisa dipakai.')
    context.application.create_task(
        _refresh_total_coin_job(context, update.effective_chat.id)
    )


async def _refresh_total_coin_job(context: ContextTypes.DEFAULT_TYPE, admin_chat_id: int):
    try:
        res = await db_exec(lambda: supabase.table('users').select('user_id, kith_coins, total_kith_coins').execute())
        updated = 0
        for user in res.data or []:
            total = user.get('total_kith_coins')
            current = user.get('kith_coins', 0)
            if total is None or total == 0:
                await db_exec(lambda user=user, current=current: supabase.table('users').update({'total_kith_coins': current}).eq('user_id', user['user_id']).execute())
                updated += 1
                await asyncio.sleep(0.02)
        await context.bot.send_message(admin_chat_id, f'✅ Refresh total coin selesai!\nUser diupdate: {updated}')
    except Exception as e:
        logger.error(f'Error refresh total coin: {e}')
        await context.bot.send_message(admin_chat_id, f'❌ Error refresh total coin: {h(e)}', parse_mode='HTML')
async def refresh_coin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.id != ADMIN_GROUP_ID:
        return

    await update.message.reply_text('⏳ Kalkulasi Kith-Coins retroaktif dimulai di background. Bot tetap bisa dipakai.')
    context.application.create_task(
        _refresh_coin_job(context, update.effective_chat.id)
    )


async def _refresh_coin_job(context: ContextTypes.DEFAULT_TYPE, admin_chat_id: int):
    try:
        response = await db_exec(lambda: supabase.table('menfess_map').select('sender_user_id').execute())
        if not hasattr(response, 'data') or not response.data:
            return await context.bot.send_message(admin_chat_id, '❌ Data menfess map masih kosong.')
        ...
        for idx, (uid, count) in enumerate(user_counts.items(), start=1):
            reward_coins = count * 50
            try:
                user_res = await db_exec(lambda uid=uid: supabase.table('users').select('kith_coins, total_kith_coins').eq('user_id', uid).execute())
                ...
                await db_exec(lambda uid=uid, new_balance=new_balance, new_total=new_total: supabase.table('users').update({'kith_coins': new_balance, 'total_kith_coins': new_total}).eq('user_id', uid).execute())

                notif_text = (
                    '🎉 <b>Kejutan Kith-Coins Retroaktif!</b>\n\n'
                    f'Terima kasih atas loyalitas kamu! Karena kamu sudah pernah mengirim <b>{count} menfess</b> di Kitheons sebelumnya, '
                    f'kamu berhak mendapatkan kompensasi sebesar <b>{reward_coins} Kith-Coins</b>!\n\n'
                    f'🪙 Saldo Koin kamu sekarang: <b>{new_balance}</b>\n\n'
                    'Koin ini bisa kamu tukarkan ke berbagai fitur mendatang seperti <b>Custom Title Loyalty</b> dan lain-lain. Pantengin terus update dari admin ya!'
                )
            ...
            if idx % 100 == 0:
                try:
                    await context.bot.send_message(admin_chat_id, f'📊 Progress refreshcoin: {idx}/{total} | Berhasil: {berhasil}, Gagal: {gagal}')
                except Exception:
                    pass

        await context.bot.send_message(admin_chat_id, f'✅ Refresh Coin Selesai!\n\n👤 User berhasil diproses: {berhasil}\n❌ Gagal kirim: {gagal}')
    except Exception as e:
        logger.error(f'Error refresh coin: {e}')
        await context.bot.send_message(admin_chat_id, '❌ Terjadi kesalahan saat memproses data database.')
def build_application():
    builder = Application.builder().token(BOT_TOKEN).post_init(on_startup)
    for method_name, value in (('connection_pool_size', 512), ('pool_timeout', 30)):
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if effective_message:
            await effective_message.reply_text('❌ Terjadi error internal. Admin sudah menerima log error.')
    except Exception:
        pass
...
def main():
    ...
    conv_handler = ConversationHandler(entry_points=[MessageHandler(filters.TEXT & filters.ChatType.PRIVATE & ~filters.COMMAND, handle_pesan)], states={WAITING_USERNAME: [MessageHandler(filters.TEXT & filters.ChatType.PRIVATE & ~filters.COMMAND, handle_username)]}, fallbacks=[CommandHandler('cancel', cancel_menfess)])
